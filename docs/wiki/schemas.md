...
- **Organization / Location / Taxonomy** — layered inheritance; `DisplayOrganization` is the common base with two branches off it: `Organization` (adds `taxonomy`) and `DetailedOrganization → OrganizationDetails` (adds `locationCount`/`userCount`/`roleCount`). `LocationCreateRequest → LocationBaseResponse → Location`; `Taxonomy` is dual-use. `LocationBaseResponse.organization` is typed as `Organization` (not `DisplayOrganization`), so location responses carry the nested `taxonomy` — which means the service must eager-load `Organization.taxonomy`.
